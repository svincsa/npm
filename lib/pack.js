    ...
    return pinflight(target, () => {
      if (mani._requested.type === 'directory') {
        return prepareDirectory(mani._resolved).then(() => {
          return packDirectory(mani, mani._resolved, target, target)
        })
...
function packFromPackage (arg, target, filename) {
  ...
  .then((pkg) => logContents(pkg, target, filename))
  .then(() => filename)
}
...
function packDirectory (mani, dir, target, filename) {
  deprCheck(mani)
  return readJson(path.join(dir, 'package.json')).then((pkg) => {
    ...
    return cacache.tmp.withTmp(npm.tmp, {tmpPrefix: 'packing'}, (tmp) => {
      ...
      //       and any such future "features" by prepending `./`
        ...
        .then(() => logContents(pkg, tmpTarget, filename))
        .then(() => (
          npm.config.get('dry-run')
            ? log.verbose('pack', '--dry-run mode enabled. Skipping write.')
            : move(tmpTarget, target, {Promise: BB, fs})
        ))
...
function logContents (pkg, target, filename) {
  ...
  .then((stat) => {
    ...
    if (bundled.size) {
      ...
      for (let name of bundled.values()) {
        log.notice('', name)
      }
    }
    ...
    log.notice('', columnify([
      ...
      {name: 'package size:', value: stat.size},
      {name: 'total files:', value: totalEntries}
    ].filter((x) => x), {
      include: ['name', 'value'],
      showHeaders: false
    }))
