...
function fetchAndRead (nv, args, silent, cb) {
  ...
  mapToRegistry(name, npm.config, function (er, uri, auth) {
    ...
    npm.registry.get(uri, { auth: auth }, function (er, data) {
      ...
      let manifest
      if (data['dist-tags']) {
        manifest = pickManifest(data, nv.fetchSpec || npm.config.get('tag'), {
      ...
      if (error || silent) {
        ...
        prettyView(data, manifest).nodeify(cb).then(() => retval)
      } else {
...
function prettyView (packument, manifest) {
  ...
  return BB.try(() => {
    ...
    const info = {
      name: color.blue(packument.name),
      version: color.blue(packument.version),
      ...
      repo: (
        packument.bugs && (packument.bugs.url || packument.bugs)
      ) || (
        packument.repository && (packument.repository.url || packument.repository)
      ),
      site: (
        packument.homepage && (packument.homepage.url || packument.homepage)
      ),
